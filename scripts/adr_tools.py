...
def cmd_new(args):
    ...
    if args.series == "numeric":
        ...
        sys.exit("Refusing to create ADR with --series all. Use --series numeric or a prefix like ECO.")
    else:
    ...
    tmpl = {"full": "_template-full.md", "lite": "_template-lite.md", "amend": "_template-amend.md"}[args.type]
...
def cmd_reindex(args):
    ...
    for p, prefix, n in sorted(iter_adr_files(args.series), key=lambda t: ((t[1] or "ADR"), t[2])):
        ...
        # If series is a single prefix (e.g., ECO), include impacted column; otherwise keep old shape.
...
if __name__ == "__main__":
    ...
    pnew.add_argument(
        "--series",
        default="numeric",
        help='Which ADR series to use: "numeric" (0001-*), "ECO" (ECO-0001-*), "INT", or "all" (index only).',
    )
